...
namespace Iot.Device.Swarm
{
    ...
    public class SwarmTile : IDisposable
    {
        ...
        internal string CommandInExecution = string.Empty;
        ...
        internal object CommandProcessedReply = new object();
        ...
        /// </summary>
        /// <param name="portName">The port where the Swarm Tile is connected to (for example, "/dev/ttyAMA0").</param>
        public SwarmTile(string portName)
        {
            // configure SerialPort and...
            TileSerialPort = new SerialPort(portName);
            TileSerialPort.Encoding = Encoding.UTF8;
            TileSerialPort.BaudRate = 115200;
            TileSerialPort.ReadTimeout = 1000;
            TileSerialPort.WriteTimeout = 1000;
        ...
        {
            // discard event if there is nothing to read
            if (TileSerialPort.BytesToRead == 0)
            {
                return;
            }
        ...
        public bool TryToSendMessage(MessageToTransmit message, out string messageId)
        {
            lock (CommandLock)
            {
                ...
                if (eventSignaled)
                {
                    ...
                    if (ErrorOccurredWhenProcessingCommand)
                    {
                        // update the error message, if available
